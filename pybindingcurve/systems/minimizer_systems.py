...
from mpmath import mpf,  power, findroot, mp
mp.dps = 100
...
def system01_minimizer(p,l,kdpl):
    ...
    p_f, l_f = findroot(f, [p*0.5,l*.5],  tol=1e-100)
...
def system02_minimizer(p,l,i,kdpl, kdpi):
    ...
    p_f, l_f, i_f = findroot(f, [p*0.5,l*.5,i*0.5], tol=1e-100, a=10)
    return {'pf':p_f,'lf':l_f,'if':i_f,'pl':(p_f*l_f)/kdpl, 'pi':(p_f*i_f)/kdpi}
...
def system03_minimizer(p,kdpp):
    ...
    def f(p_f):
        pp=p_f*p_f/kdpp
        return p-(p_f+2*pp)
    p_f = findroot(f, p*0.5,  tol=1e-100)
...
def system04_minimizer(p,i,kdpp,kdpi):
    ...
    p_f,i_f = findroot(f, [p*0.5,i*0.5],  tol=1e-100)
...
def system12_minimizer(p,l,kdpl1, kdpl2):
    ...
    pl1=(p_f*l_f)/kdpl1
    pl2=(p_f*l_f)/kdpl2
    return {'pf':p_f,'lf':l_f,'pl1':pl1, 'pl2':pl2, 'pl12':(pl1*l_f+pl2*l_f)/(kdpl1+kdpl2) }

# 1:3 binding
def system12_minimizer(p,l,kdpl1, kdpl2, kdpl3):
    p=mpf(p)
    l=mpf(l)
    kdpl1=mpf(kdpl1)
    kdpl2=mpf(kdpl2)
    ...
    def f(p_f,l_f):
        ...
        return p-(p_f+pl1+pl2+pl3+pl12+pl13+pl23), l-(l_f+pl1+pl2+pl3+2*(pl12+pl13+pl23)+3*pl123)
    p_f, l_f = findroot(f, [p*0.5,l*.5],  tol=1e-100)
    pl1=p_f*l_f/kdpl1
    pl2=p_f*l_f/kdpl2
    pl3=p_f*l_f/kdpl3
    ...
    pl23=(pl2*l_f+pl3*l_f)/(kdpl2+kdpl3)
    pl13=(pl1*l_f+pl3*l_f)/(kdpl1+kdpl3)
    return {'pf':p_f,'lf':l_f,'pl1':pl1, 'pl2':pl2, 'pl3':pl3, 'pl12':pl12, 'pl13':pl13,'pl23':pl23, 'pl123':(pl12*l_f+pl23*l_f+pl13*l_f)/(kdpl1+kdpl2+kdpl3)}
