...
for i in range(len(heatmaps_hetero)):
    ...
    sns.heatmap(heatmaps_homo[i][:,::-1], ax=ax[0], vmin=0, vmax=1,
                cmap=colour_map, cbar=True, cbar_kws=dict(pad=0.01))
    sns.heatmap(heatmaps_hetero[i][:,::-1], ax=ax[1], vmin=0, vmax=1,
                cmap=colour_map, cbar=True, cbar_kws=dict(pad=0.01))
    sns.heatmap(heatmaps_homo[i][:,::-1]-heatmaps_hetero[i][:,::-1], ax=ax[2],
                cmap=sns.color_palette("PRGn", 1024), center=0, cbar=True, cbar_kws=dict(pad=0.01))
...
for axx in ax:
    plt.xticks(
        np.arange(0, heatmaps_hetero[0].shape[1]+1, heatmaps_hetero[0].shape[1]/2), rotation=0)
    plt.yticks(
