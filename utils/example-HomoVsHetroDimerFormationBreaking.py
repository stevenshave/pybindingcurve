...
for i in range(len(homo_y_formation)):
    homo_y_formation[i] = pbc.systems.System_analytical_homodimerformation_pp().query(
        {"kdpp": dimer_kd, "p": x_axis_formation[i] * 2}
    )
    hetero_y_formation[i] = pbc.systems.System_analytical_one_to_one_pl().query(
        {"kdpl": dimer_kd, "p": x_axis_formation[i], "l": x_axis_formation[i]}
    )

homo_y_breaking = np.empty((num_points))
hetero_y_breaking = np.empty((num_points))
for i in range(len(homo_y_formation)):
    homo_y_breaking[i] = pbc.systems.System_kinetic_homodimerbreaking_pp().query(
        {"kdpi": inhibitor_kd, "kdpp": dimer_kd, "p": 2, "i": x_axis_breaking[i]}
    )
    hetero_y_breaking[i] = pbc.systems.System_analytical_competition_pl().query(
        {
            "kdpi": inhibitor_kd,
            "kdpl": dimer_kd,
            "p": 1,
...
ax[0].plot(
    x_axis_formation, homo_y_formation, "k", label="Homodimer monomers", linestyle="--"
)
